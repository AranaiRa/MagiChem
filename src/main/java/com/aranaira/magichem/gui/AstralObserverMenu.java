...
import static com.aranaira.magichem.block.entity.AstralObserverBlockEntity.*;

public class AstralObserverMenu extends AbstractContainerMenu {
    public final AstralObserverBlockEntity blockEntity;
    private final Level level;

    public AstralObserverMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(1));
    }

    public AstralObserverMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(MenuRegistry.ASTRAL_OBSERVER_MENU.get(), id);
        blockEntity = (AstralObserverBlockEntity) entity;
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, BlockRegistry.ASTRAL_OBSERVER.get());
    }
}
