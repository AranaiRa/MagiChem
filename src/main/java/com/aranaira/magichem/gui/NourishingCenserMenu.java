...
public class NourishingCenserMenu extends AbstractContainerMenu {
    public final NourishingCenserBlockEntity blockEntity;
    private final Level level;

    public NourishingCenserMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(1));
    }

    public NourishingCenserMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(MenuRegistry.NOURISHING_CENSER_MENU.get(), id);
        blockEntity = (NourishingCenserBlockEntity) entity;
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, BlockRegistry.NOURISHING_CENSER.get());
    }
}
