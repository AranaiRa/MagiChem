...
import static com.aranaira.magichem.block.entity.SkywrathCondenserBlockEntity.*;

public class SkywrathCondenserMenu extends AbstractContainerMenu {
    public final SkywrathCondenserBlockEntity blockEntity;
    private final Level level;

    public SkywrathCondenserMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(1));
    }

    public SkywrathCondenserMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(MenuRegistry.SKYWRATH_CONDENSER_MENU.get(), id);
        blockEntity = (SkywrathCondenserBlockEntity) entity;
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            this.addSlot(new SlotItemHandler(handler, SLOT_MATERIA, 115, 14));
            this.addSlot(new SlotItemHandler(handler, SLOT_BOTTLES, 115, 40));
        });
    }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, BlockRegistry.SKYWRATH_CONDENSER.get());
    }
}
