...
public class MateriaJarQuadItem extends BlockItem {
    public MateriaJarQuadItem(Block pBlock, Properties pProperties) {
    ...
    public InteractionResult place(BlockPlaceContext context) {
        ...
        if(result == InteractionResult.CONSUME) {
            if (!context.getLevel().isClientSide() && tag != null) {
                BlockPos clickedPos = context.getClickedPos();
                BlockEntity entity = context.getLevel().getBlockEntity(clickedPos);
                if (entity instanceof MateriaJarQuadBlockEntity mjqbe) {
                    for(int i=0; i<4; i++) {
                        if(tag.contains("materiaType"+i)) {
                            CompoundTag entry = tag.getCompound("materiaType"+i);
                            if (entry.contains("type") && entry.contains("count")) {
                                ...
                                mjqbe.setContents(i, materia, entry.getInt("count"));
                            }
                        }
                    }
